...
/** Return first validation failure message or null */
...
router.post('/register/farmer', async (req, res, next) => {
  try {
    ...
    if (!phone_number || !profile_name || !password) {
      return res.status(400).json({ success: false, message: 'Phone, name, and password are required' });
    }

    // Regex validation
    const err = validate([
      [V.name.test(profile_name),                          'Name: only letters & spaces, 2–50 chars'],
      [V.phone.test(phone_number),                         'Phone: must be 10 digits starting with 6-9'],
      [V.password.test(password),                          'Password: min 8 chars, 1 upper, 1 lower, 1 digit, 1 special char'],
      [!aadhaar_number || V.aadhaar.test(aadhaar_number),  'Aadhaar: must be exactly 12 digits'],
      [!bank_account_number || V.bankAccount.test(bank_account_number), 'Bank account: must be 9–18 digits'],
      [!bank_ifsc || V.ifsc.test(bank_ifsc),               'IFSC: invalid format (e.g. SBIN0001234)'],
    ]);
...
router.post('/register/customer', async (req, res, next) => {
  try {
    ...
    if (!email || !profile_name || !password || !company_name) {
      return res.status(400).json({ success: false, message: 'Email, name, password, and company name are required' });
    }

    // Regex validation
    const err = validate([
      [V.name.test(profile_name),                'Name: only letters & spaces, 2–50 chars'],
      [V.email.test(email),                      'Email: invalid format'],
      [V.password.test(password),                'Password: min 8 chars, 1 upper, 1 lower, 1 digit, 1 special char'],
      [!phone_number || V.phone.test(phone_number), 'Phone: must be 10 digits starting with 6-9'],
      [!gst_number || V.gst.test(gst_number),   'GST: invalid format (e.g. 22AAAAA0000A1Z5)'],
    ]);
    if (err) return res.status(400).json({ success: false, message: err });
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    if (email && !V.email.test(email)) return res.status(400).json({ success: false, message: 'Email: invalid format' });
    if (phone_number && !V.phone.test(phone_number)) return res.status(400).json({ success: false, message: 'Phone: must be 10 digits starting with 6-9' });
